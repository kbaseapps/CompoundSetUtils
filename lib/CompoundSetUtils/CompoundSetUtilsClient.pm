package CompoundSetUtils::CompoundSetUtilsClient;
...
CompoundSetUtils::CompoundSetUtilsClient
...
sub new
{
    ...
    my $self = {
	client => CompoundSetUtils::CompoundSetUtilsClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: required
	...
	my @_bad_arguments;
	    ...
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'compound_set_from_file');
	}
    }
	...
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method compound_set_from_file",
...
# Authentication: required
	...
	my @_bad_arguments;
	    ...
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'compound_set_to_file');
	}
    }

    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "CompoundSetUtils.compound_set_to_file",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method compound_set_to_file",
					    status_line => $self->{client}->status_line,
					    method_name => 'compound_set_to_file',
				       );
    }
...
# Authentication: required
	...
	my @_bad_arguments;
	    ...
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'compound_set_from_model');
	}
    }

    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "CompoundSetUtils.compound_set_from_model",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method compound_set_from_model",
					    status_line => $self->{client}->status_line,
					    method_name => 'compound_set_from_model',
				       );
    }
...
=head2 export_compoundset_as_tsv

  $output = $obj->export_compoundset_as_tsv($params)
...
<pre>
$params is a CompoundSetUtils.ExportParams
$output is a CompoundSetUtils.ExportOutput
...
# Authentication: required
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function export_compoundset_as_tsv (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to export_compoundset_as_tsv:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'export_compoundset_as_tsv');
	}
    }

    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "CompoundSetUtils.export_compoundset_as_tsv",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method export_compoundset_as_tsv",
					    status_line => $self->{client}->status_line,
					    method_name => 'export_compoundset_as_tsv',
				       );
    }
...
=head2 export_compoundset_as_sdf

  $output = $obj->export_compoundset_as_sdf($params)
...
# Authentication: required
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function export_compoundset_as_sdf (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to export_compoundset_as_sdf:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'export_compoundset_as_sdf');
	}
    }

    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "CompoundSetUtils.export_compoundset_as_sdf",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method export_compoundset_as_sdf",
					    status_line => $self->{client}->status_line,
					    method_name => 'export_compoundset_as_sdf',
				       );
    }
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "CompoundSetUtils.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method export_compoundset_as_sdf",
            status_line => $self->{client}->status_line,
            method_name => 'export_compoundset_as_sdf',
        );
    }
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for CompoundSetUtils::CompoundSetUtilsClient\n";
    }
    if ($sMajor == 0) {
        warn "CompoundSetUtils::CompoundSetUtilsClient version is $svr_version. API subject to change.\n";
    }
}
...
=head2 compoundset_download_results
...
package CompoundSetUtils::CompoundSetUtilsClient::RpcClient;
